            ...
            this.getPublishPair(function(error, p1, p2) {
                if (error) {
                    ...
                    var killTimeout = setTimeout(function() {
                        if (n) {
                            n.kill('SIGTERM');
                            db.query('INSERT INTO tw_battles(publish1_id, publish2_id, winner_id, battle_result, battle_time, battle_replay) VALUES(?, ?, 0, "timeout", now(), "")',
                                [p1, p2], function() {
                                    self.running = false;
                                }
                            );
                    ...
                    n.on('message', function(result) {
                        ...
                        db.query('INSERT INTO tw_battles(publish1_id, publish2_id, winner_id, battle_result, battle_time, battle_replay) VALUES(?, ?, ?, ?, now(), ?)',
